...
func HasWriteAccess(tagName string) (bool, error) {
	...
	recordingTransport := &artifactoryErrorRecordingTransport{}
	...
	if err != nil {
		if transportError, ok := err.(*transport.Error); ok {
			...
			if recordingTransport.isArtifactoryUnauthorized() {
				return false, nil
			}
		}
...
type artifactoryErrorRecordingTransport struct {
	isToken401 bool
}

func (h *artifactoryErrorRecordingTransport) RoundTrip(r *http.Request) (*http.Response, error) {
...
func (h *artifactoryErrorRecordingTransport) isArtifactoryUnauthorized() bool {
	return h.isToken401
}
